 ...
 * 
 * Provides functionality to search through ADR titles, content, and filenames
...
export class SearchService implements ISearchService {
   ...
   * @private
   */
  private filterDirectory(directory: ADRDirectory, searchTerms: string[]): ADRDirectory {
    ...
    const filteredSubdirectories = directory.subdirectories
      ...
      .filter(subdir => subdir.adrs.length > 0 || subdir.subdirectories.length > 0);
   ...
   * @returns Lowercased concatenated text from title, content, and filename
   * @private
   */
